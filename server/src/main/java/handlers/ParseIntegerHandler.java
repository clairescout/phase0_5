...
public class ParseIntegerHandler extends HandlerBase {

    @Override
    public void handle(com.sun.net.httpserver.HttpExchange exchange) throws IOException{
        System.out.println("in parse integer handler");
        ServerSerializer serverSerializer = new ServerSerializer();
        boolean success = false;
        try{
            if(exchange.getRequestMethod().toLowerCase().equals("post")){
                RequestData requestData = getRequestData(exchange);
                Results r = StringProcessor.getInstance().parseInteger(requestData.getData());
